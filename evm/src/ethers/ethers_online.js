...
const sendTx = async (wallet, txRaw, waitFlag = true) => {
    ...
    // 等待交易被打包
    const txReceipt_2 = await txReceipt_1.wait();
    // console.log(txReceipt_2);

    const { blockNumber } = txReceipt_2;
    ...
    return JSON.stringify({
        blockNumber, gasUsed, effectiveGasPrice_Gwei, tx_fee_eth,
    });
};
...
const getNetGasPrice = async (provider) => {
    ...
    return JSON.stringify({
        recommendGasPrice, lastBaseFeePerGas, maxFeePerGas, maxPriorityFeePerGas,
    });
};
...
const transferExact = async (wallet, to_address, value_ether, waitFlag = true, maxFeePerGas_gwei = '10', maxPriorityFeePerGas_gwei = '1') => {
    ...
    try {
        const jsonResult = await sendTx(wallet, rawTx, waitFlag);
        return jsonResult;
    } catch (e) {
...
export {
    initWallet, getBalance, getNetGasPrice, getProviderStatus, loopGetTargetGasPrice,
};
